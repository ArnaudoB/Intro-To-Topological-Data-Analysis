...
def test_simplexes():
    """
    Writes some basic tests for the simplexes function and plots the results.
    """
    ...
    tuples = sorted(list(dic.keys()), key=tuple)
    filtrations = sorted(list(dic.values()))
    assert tuples == [(0,), (0, 1), (0, 2), (1,), (2,)] and np.allclose(filtrations, [0, 0, 0, 0.5, 0.5], rtol=1e-5, atol=1e-5), "Test failed"
    ...
    assert (tuples==[(0,), (0, 1), (0, 1, 2), (0, 1, 3), (0, 2), (0, 2, 3), (0, 3), (1,), (1, 2), (1, 3), (2,), (2, 3), (3,)]
            and np.allclose(filtrations, [0, 0, 0, 0, 0.5, 0.5, 0.5, np.sqrt(2)/2, np.sqrt(2)/2, np.sqrt(2)/2, np.sqrt(2)/2, np.sqrt(2)/2, np.sqrt(2)/2], rtol=1e-5, atol=1e-5)), "Test failed"
